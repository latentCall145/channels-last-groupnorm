...
spatial_loop(
  ...
  /*
     ...
     C <= MAX_THREADS_PER_BLOCK (Kernel 1):
       griddim: (x=N, y=f=1); blockdim: (x=C, y=d)
     ...
     C > MAX_THREADS_PER_BLOCK (Kernel 2):
       griddim: (x=N, y=f); blockdim: (x=TPB, y=d=1)
        f = factor of channels that each thread have to process separately
        d = num. spatial elements (from HW dimension) each thread-block processes in parallel
        f * TPB = C
...
#pragma unroll 8
  for (int i = 0; i < Hw; ++i) {
    ...
    //reduce_idx += blockIdx.z * TPB; // dim 3, TPB stride (in kernel 1, threadIdx.z is always 0 so this statement does nothing) TODO: uncomment
...
compute_bwd_scale_biases(
  ...
  idx += d * G;
  idx += g;
  //vals_reduced[2 * tid] = xdy_sum_data[idx] * gamma_v;
  //vals_reduced[2 * tid + 1] = dy_sum_data[idx] * gamma_v;
...
void gn_bwd(
  ...
  const auto kAccType =
      ...
      ? at::kFloat
      : X_nhwc.scalar_type();
  torch::Tensor xdy_sum = at::empty({N, C}, X_nhwc.options().dtype(kAccType));
  ...
  spatial_loop<<<dimGrid, dimBlock, sizeof(T) * 2 * TPB>>>(
      dy_data, X_data, 
      H, W, C,
